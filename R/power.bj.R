...
#' @param n - dimension parameter, i.e. the number of input statitics to construct B-J statistic.
...
#' alpha_phi <- 1 - pphi(q=bjstat, n=10, s=1, beta=0.5)
#' #If the alternative hypothesis Gaussian mixture with eps = 0.1 and mu = 1.2:#
#' power.bj(alpha_phi, 10, 0.5, eps = 0.1, mu = 1.2)
#' @export
#' @importFrom stats dpois pbeta
power.bj <- function(alpha, n, beta, method = "gaussian-gaussian",
                      eps=0, mu=0, df=1, delta=0){
  return(power.phi(alpha, n, 1, beta, method, eps, mu, df, delta))
}
